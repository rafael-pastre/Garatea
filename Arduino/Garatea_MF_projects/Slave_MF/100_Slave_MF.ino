...
uint8_t transmissionState=0;						// Controls which data slave will send to master
...
*******************************************************************************/
void setup() {
	// PC Serial initialization
	Serial.begin(DEBUG_BAUD);
	Serial.println(F("Slave monitor:"));
	...
	Wire.onRequest(requestEvent);                   // defines the event(function) for requesting data in I2C bus
	Wire.onReceive(receiveEvent);                   // defines the event(function) for recieving data in I2C bus
}
